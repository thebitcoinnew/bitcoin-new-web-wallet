...
export class WalletService {
  ...
  async processStateBlock(transaction) {
    ...
    if (transaction.is_send === 'true' && transaction.block.link_as_account) {
      ...
      if (this.wallet.pending.lte(0)) {
        ...
        this.wallet.pendingRaw = this.wallet.pendingRaw.plus(txAmount.mod(this.btcnew));
        this.wallet.pendingFiat += this.util.btcnew.rawToMbtcnew(txAmount).times(this.price.price.lastPrice).toNumber();
        this.wallet.hasPending = true;
      }

      if (this.appSettings.settings.minimumReceive) {
        const minAmount = this.util.btcnew.mbtcnewToRaw(this.appSettings.settings.minimumReceive);
  ...
  reloadFiatBalances() {
    ...
    this.wallet.accounts.forEach(account => {
      account.balanceFiat = this.util.btcnew.rawToMbtcnew(account.balance).times(fiatPrice).toNumber();
      account.pendingFiat = this.util.btcnew.rawToMbtcnew(account.pending).times(fiatPrice).toNumber();
    });

    this.wallet.balanceFiat = this.util.btcnew.rawToMbtcnew(this.wallet.balance).times(fiatPrice).toNumber();
    this.wallet.pendingFiat = this.util.btcnew.rawToMbtcnew(this.wallet.pending).times(fiatPrice).toNumber();
  }
  ...
  async reloadBalances(reloadPending = true) {
    ...
    for (const accountID in accounts.balances) {
      ...
      walletAccount.balanceRaw = new BigNumber(walletAccount.balance).mod(this.btcnew);
      walletAccount.pendingRaw = new BigNumber(walletAccount.pending).mod(this.btcnew);

      walletAccount.balanceFiat = this.util.btcnew.rawToMbtcnew(walletAccount.balance).times(fiatPrice).toNumber();
      walletAccount.pendingFiat = this.util.btcnew.rawToMbtcnew(walletAccount.pending).times(fiatPrice).toNumber();
    ...
    this.wallet.balanceRaw = new BigNumber(walletBalance).mod(this.btcnew);
    this.wallet.pendingRaw = new BigNumber(walletPending).mod(this.btcnew);

    this.wallet.balanceFiat = this.util.btcnew.rawToMbtcnew(walletBalance).times(fiatPrice).toNumber();
    this.wallet.pendingFiat = this.util.btcnew.rawToMbtcnew(walletPending).times(fiatPrice).toNumber();
  ...
  async processPendingBlocks() {
    ...
    const newHash = await this.btcnewBlock.generateReceive(walletAccount, nextBlock.hash, this.isLedgerWallet());
    if (newHash) {
      if (this.successfulBlocks.length >= 15) this.successfulBlocks.shift();
      this.successfulBlocks.push(nextBlock.hash);

      const receiveAmount = this.util.btcnew.rawToMbtcnew(nextBlock.amount);
      this.notifications.sendSuccess(`Successfully received ${receiveAmount.isZero() ? '' : receiveAmount.toFixed(6)} Btcnew!`);
