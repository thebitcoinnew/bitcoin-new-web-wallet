...
import Btcnew from "hw-app-nano";
...
export class LedgerService {
  ...
  // Scraps binding to any existing transport/btcnew object
  resetLedger() {
    this.ledger.transport = null;
    this.ledger.btcnew = null;
  }
  ...
  async loadLedger(hideNotifications = false) {
    return new Promise(async (resolve, reject) => {
      ...
      // Load btcnew object
      if (!this.ledger.btcnew) {
        try {
          this.ledger.btcnew = new Btcnew(this.ledger.transport);
        } catch (err) {
          console.log(`Btcnew error: `, err);
          if (err.statusText == 'UNKNOWN_ERROR') {
            this.resetLedger();
          }
          this.ledgerStatus$.next({ status: this.ledger.status, statusText: `Error loading Btcnew USB transport` });
          return resolve(false);
        }
      }
      ...
      setTimeout(() => {
        ...
        this.ledgerStatus$.next({ status: this.ledger.status, statusText: `Unable to detect Btcnew Ledger application (Timeout)` });
        if (!hideNotifications) {
          this.notifications.sendWarning(`Unable to connect to the Ledger device.  Make sure it is unlocked and the Btcnew application is open`);
        }
        resolved = true;
        return resolve(false);
      }, 2500);
      ...
      try {
        const ledgerConfig = await this.ledger.btcnew.getAppConfiguration();
        resolved = true;
        ...
        if (ledgerConfig && ledgerConfig.version) {
          ...
          this.ledgerStatus$.next({ status: this.ledger.status, statusText: `Btcnew app detected, but ledger is locked` });
        }
      } catch (err) {
        ...
        if (!hideNotifications && !resolved) {
          this.notifications.sendWarning(`Ledger device locked.  Unlock and open the Btcnew application`);
        }
        return resolve(false);
      }
      ...
      try {
        ...
        this.ledgerStatus$.next({ status: this.ledger.status, statusText: `Btcnew Ledger application connected` });
  ...
  async updateCache(accountIndex, blockHash) {
    ...
    if (this.isDesktop) {
      ...
      return await this.ledger.btcnew.cacheBlock(this.ledgerPath(accountIndex), cacheData, blockData.contents.signature);
    }
  }
  ...
  async signBlock(accountIndex: number, blockData: any) {
    ...
    if (this.isDesktop) {
      ...
      return await this.ledger.btcnew.signBlock(this.ledgerPath(accountIndex), blockData);
    }
  }
  ...
  async getLedgerAccountWeb(accountIndex: number, showOnScreen = false) {
    ...
    try {
      return await this.ledger.btcnew.getAddress(this.ledgerPath(accountIndex), showOnScreen);
    } catch (err) {
      throw err;
    }
  }
