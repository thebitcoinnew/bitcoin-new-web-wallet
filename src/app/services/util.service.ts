...
export class UtilService {
  ...
  btcnew = {
    mbtcnewToRaw: mbtcnewToRaw,
    kbtcnewToRaw: kbtcnewToRaw,
    btcnewToRaw: btcnewToRaw,
    rawToMbtcnew: rawToMbtcnew,
    rawToKbtcnew: rawToKbtcnew,
    rawToBtcnew: rawToBtcnew,
  };
...
function getPublicAccountID(accountPublicKeyBytes, prefix = 'btcnew') {
...
function getAccountPublicKey(account) {
  if (account.length == 67) {
    if(!account.startsWith('btcnew_1') && !account.startsWith('btcnew_3')) {
      throw new Error(`Invalid BTCNEW Account`);
    }
  } else {
    throw new Error(`Invalid BTCNEW Account`);
  }
  const account_crop = account.substring(7,67);
  const isValid = /^[13456789abcdefghijkmnopqrstuwxyz]+$/.test(account_crop);
  if (!isValid) throw new Error(`Invalid BTCNEW account`);
...
function setPrefix(account, prefix = 'btcnew') {
  if (prefix === 'btcnew') {
    return account.replace('xrb_', 'btcnew_');
  } else {
    return account.replace('btcnew_', 'xrb_');
  }
}
 ...
 */
const mbtcnew = 1000000000000000000000000000000;
const kbtcnew = 1000000000000000000000000000;
const btcnew  = 1000000000000000000000000;
function mbtcnewToRaw(value) {
  return new BigNumber(value).times(mbtcnew);
}
function kbtcnewToRaw(value) {
  return new BigNumber(value).times(kbtcnew);
}
function btcnewToRaw(value) {
  return new BigNumber(value).times(btcnew);
}
function rawToMbtcnew(value) {
  return new BigNumber(value).div(mbtcnew);
}
function rawToKbtcnew(value) {
  return new BigNumber(value).div(kbtcnew);
}
function rawToBtcnew(value) {
  return new BigNumber(value).div(btcnew);
}
