...
export class RaiPipe implements PipeTransform {
  ...
  transform(value: any, args?: any): any {
    ...
    switch (denomination.toLowerCase()) {
      default:
      case 'xrb': return `${(value / this.mrai).toFixed(6)}${!hideText ? ' BTCNEW': ''}`;
      case 'mbtcnew':
        ...
        if (hasRawValue) {
          ...
          return `${this.toFixed(newVal, this.precision)}${!hideText ? ' BTCNEW': ''}`;
        } else {
          return `${(value / this.mrai).toFixed(6)}${!hideText ? ' BTCNEW': ''}`;
        }
      case 'kbtcnew': return `${(value / this.krai).toFixed(3)}${!hideText ? ' kbtcnew': ''}`;
      case 'btcnew': return `${(value / this.rai).toFixed(0)}${!hideText ? ' btcnew': ''}`;
