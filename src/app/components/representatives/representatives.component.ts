...
export class RepresentativesComponent implements OnInit {
  ...
  async loadRecommendedReps() {
    ...
    try {
      ...
      const reps = scores.map(rep => {
        const btcnewWeight = this.util.btcnew.rawToMbtcnew(rep.votingweight.toString() || 0);
        const percent = btcnewWeight.div(totalSupply).times(100);

        // rep.weight = btcnewWeight.toString(10);
        rep.weight = this.util.btcnew.mbtcnewToRaw(btcnewWeight);
  ...
  async changeRepresentatives() {
    ...
    for (let account of accountsNeedingChange) {
      ...
      try {
        const changed = await this.btcnewBlock.generateChange(walletAccount, newRep, this.wallet.isLedgerWallet());
        if (!changed) {
