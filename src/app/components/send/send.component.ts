...
import {BtcnewBlockService} from "../../services/btcnew-block.service";
...
export class SendComponent implements OnInit {
  ...
  amounts = [
    { name: 'BTCNEW (1 Mbtcnew)', shortName: 'BTCNEW', value: 'mbtcnew' },
    { name: 'kbtcnew (0.001 Mbtcnew)', shortName: 'kbtcnew', value: 'kbtcnew' },
    { name: 'btcnew (0.000001 Mbtcnew)', shortName: 'btcnew', value: 'btcnew' },
  ];
  ...
  constructor(
    ...
    private btcnewBlock: BtcnewBlockService,
  ...
  // An update to the Btcnew amount, sync the fiat value
  syncFiatPrice() {
    ...
    const fiatAmount = this.util.btcnew.rawToMbtcnew(rawAmount).times(this.price.price.lastPrice).times(precision).floor().div(precision).toNumber();
    this.amountFiat = fiatAmount;
  }

  // An update to the fiat amount, sync the btcnew value based on currently selected denomination
  syncBtcnewPrice() {
    const fiatAmount = this.amountFiat || 0;
    const rawAmount = this.util.btcnew.mbtcnewToRaw(new BigNumber(fiatAmount).div(this.price.price.lastPrice));
    const btcnewVal = this.util.btcnew.rawToBtcnew(rawAmount).floor();
    const btcnewAmount = this.getAmountValueFromBase(this.util.btcnew.btcnewToRaw(btcnewVal));

    this.amount = btcnewAmount.toNumber();
  }
  ...
  async sendTransaction() {
    ...
    const btcnewAmount = this.rawAmount.div(this.btcnew);

    if (this.amount < 0 || rawAmount.lessThan(0)) return this.notificationService.sendWarning(`Amount is invalid`);
    if (btcnewAmount.lessThan(1)) return this.notificationService.sendWarning(`Transactions for less than 1 btcnew will be ignored by the node.  Send raw amounts with at least 1 btcnew.`);
    if (from.balanceBN.minus(rawAmount).lessThan(0)) return this.notificationService.sendError(`From account does not have enough BTCNEW`);

    // Determine a proper raw amount to show in the UI, if a decimal was entered
    this.amountRaw = this.rawAmount.mod(this.btcnew);

    // Determine fiat value of the amount
    this.amountFiat = this.util.btcnew.rawToMbtcnew(rawAmount).times(this.price.price.lastPrice).toNumber();
  ...
  async confirmTransaction() {
    ...
    try {
      ...
      // this.btcnewBlock.subscribeSend(walletAccount, this.toAccountID, this.rawAmount, this.walletService.isLedgerWallet()).subscribe(value => {
      //   console.log('GOT VALUE!!! ', value)
      // }, err => {
      //   console.log('GOT ERROR!!!: ', err);
      // });
      const newHash = await this.btcnewBlock.generateSend(walletAccount, this.toAccountID, this.rawAmount, this.walletService.isLedgerWallet());
      if (newHash) {
  ...
  setMaxAmount() {
    ...
    const btcnewVal = this.util.btcnew.rawToBtcnew(walletAccount.balance).floor();
    const maxAmount = this.getAmountValueFromBase(this.util.btcnew.btcnewToRaw(btcnewVal));
  ...
  getAmountBaseValue(value) {
    ...
    switch (this.selectedAmount.value) {
      default:
      case 'btcnew': return this.util.btcnew.btcnewToRaw(value);
      case 'kbtcnew': return this.util.btcnew.kbtcnewToRaw(value);
      case 'mbtcnew': return this.util.btcnew.mbtcnewToRaw(value);
    }
  }
  ...
  getAmountValueFromBase(value) {
    switch (this.selectedAmount.value) {
      default:
      case 'btcnew': return this.util.btcnew.rawToBtcnew(value);
      case 'kbtcnew': return this.util.btcnew.rawToKbtcnew(value);
      case 'mbtcnew': return this.util.btcnew.rawToMbtcnew(value);
    }
  }
