...
export class AccountDetailsComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    this.priceSub = this.price.lastPrice$.subscribe(event => {
      this.account.balanceFiat = this.util.btcnew.rawToMbtcnew(this.account.balance || 0).times(this.price.price.lastPrice).toNumber();
      this.account.pendingFiat = this.util.btcnew.rawToMbtcnew(this.account.pending || 0).times(this.price.price.lastPrice).toNumber();
    });
  ...
  async loadAccountDetails() {
    ...
    if ((!this.account.error && this.account.pending > 0) || this.account.error) {
      ...
      if (this.settings.settings.minimumReceive) {
        const minAmount = this.util.btcnew.mbtcnewToRaw(this.settings.settings.minimumReceive);
    ...
    // Set fiat values?
    this.account.balanceRaw = new BigNumber(this.account.balance || 0).mod(this.btcnew);
    this.account.pendingRaw = new BigNumber(this.account.pending || 0).mod(this.btcnew);
    this.account.balanceFiat = this.util.btcnew.rawToMbtcnew(this.account.balance || 0).times(this.price.price.lastPrice).toNumber();
    this.account.pendingFiat = this.util.btcnew.rawToMbtcnew(this.account.pending || 0).times(this.price.price.lastPrice).toNumber();
  ...
  async saveRepresentative() {
    ...
    try {
      const changed = await this.btcnewBlock.generateChange(this.walletAccount, repAccount, this.wallet.isLedgerWallet());
      if (!changed) {
