...
export class LedgerService {
  ...
  // Open a connection to the usb device and initialize up the Btcnew Ledger library
  async loadTransport() {
  ...
  async loadAppConfig(): Promise<any> {
    return new Promise((resolve, reject) => {
      this.ledger.btcnew.getAppConfiguration().then(resolve).catch(reject);
    })
  }
  ...
  async getLedgerAccount(accountIndex, showOnScreen = false) {
    try {
      ...
      const account = await this.ledger.btcnew.getAddress(this.ledgerPath(accountIndex), showOnScreen);
  ...
  async cacheBlock(accountIndex, cacheData, signature) {
    try {
      this.queryingLedger = true;
      const cacheResponse = await this.ledger.btcnew.cacheBlock(this.ledgerPath(accountIndex), cacheData, signature);
      this.queryingLedger = false;
  ...
  async signBlock(accountIndex, blockData) {
    try {
      this.queryingLedger = true;
      const signResponse = await this.ledger.btcnew.signBlock(this.ledgerPath(accountIndex), blockData);
      this.queryingLedger = false;
